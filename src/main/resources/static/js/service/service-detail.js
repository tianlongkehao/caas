		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data.status == "200") {
					...
					var html = '<pre class="serviceLogs" style="background: none repeat scroll 0 0 black; color: #37fc34; border: 0; font-size: 12px; overflow: hidden; float: left;">' + containerlog + '</pre>';
...
function checkSerAddr() {
	if ($('#editServiceAddr').val() === '') {
		layer.tips('服务路径不能为空', $('#editServiceAddr'), {
			tips : [1, '#EF6578']
		});
...
function saveEnvEdit(obj) {
	...
	if (1 == checkRepEnv($(obj).parent().parent().find("input.envKey"), id, envKey)) {
		return;
	};
	//环境变量Key只能是字母数字下划线；
	reg = /^[A-Za-z_][A-Za-z0-9_]*$/;
	if (!reg.test(envKey)) {
		layer.tips('环境变量key只能是字母数字下划线，不能以数字开头', $(obj).parent().parent().find("input.envKey"), {
			tips : [1, '#3595CC']
		});
		$('#newKey').focus();
		return;
	...
	if (envKey.length >= 4096) {
		layer.tips('key字符长度不能超过4096', $(obj).parent().parent().find("input.envKey"), {
			tips : [1, '#3595CC']
		});
		$('#newKey').focus();
		return;
	...
	if (envValue.length >= 4096) {
		layer.tips('value字符长度不能超过4096', $(obj).parent().parent().find("input.envValue"), {
			tips : [1, '#3595CC']
		});
		$('#newValue').focus();
		return;
...
function addEnvClick(obj) {
	layer.open({
		...
		yes : function(index, layero) {
			...
			reg = /^[A-Za-z_][A-Za-z0-9_]*$/;
			if (!reg.test(envKey)) {
				layer.tips('环境变量key只能是字母数字下划线，不能以数字开头', $("#newKey"), {
					tips : [1, '#3595CC']
				});
				$('#newKey').focus();
				return;
			...
			//判断envKey长度
			if (envKey.length >= 4096) {
				layer.tips('key字符长度不能超过4096', $("#newKey"), {
					tips : [1, '#3595CC']
				});
				$('#newKey').focus();
				return;
			...
			//判断envValue长度
			if (envValue.length >= 4096) {
				layer.tips('value字符长度不能超过4096', $$("#newValue"), {
					tips : [1, '#3595CC']
				});
				$('#newValue').focus();
				return;
			...
			$.ajax({
				...
				success : function(data) {
					...
					var newEnv = '<tr><td style="width:40%;text-indent: 15px;">' + '<span id="key' + envVariable.envId + '" class="oldEnv">' + envVariable.envKey + '</span>' + '<span class="editEnv" hidden="true"><input class="envKey" type="text" name="envKey" value="' + envVariable.envKey + '"  /></span>' + '<input class="envId" hidden="true" value="' + envVariable.envId + '" /></td>' + '<td>' + '<span id="value' + envVariable.envId + '" class="oldEnv">' + envVariable.envValue + '</span>' + ' <span class="editEnv" hidden="true"><input class="envValue" type="text" name="envValue" value="' + envVariable.envValue + '"  /></span></td>' + '<td style="width:10%;" class="editBtn">' + '<i onclick="editEnvBtn(this)"  type="button" value="修改"  class="fa fa-edit oldEnvConfig editEnvBtn"></i>' + '<i onclick="saveEnvEdit(this)" hidden=true type="button" value="提交"  class="fa fa-save editEnv saveEnv"></i>' + '<i onclick="canclEnvEdit(this)" hidden=true type="button" value="取消"  class="fa fa-times editEnv"></i>' + ' <i onclick="delEnvEdit(this)"  type="button" value="删除"  class="fa fa-trash editEnv editEnvBtn"></i>' + '</td>' + '</tr>';
...
function checkRepPortCfg(obj, id, port) {
	...
	$('#editPortCfgBody tr').each(function(index, domEle) {
		...
		if (port == pcp) {
			layer.tips('容器端口不能重复', $(obj).parent().parent().find("input.containerPort"), {
				tips : [1, '#3595CC']
			});
			$(obj).parent().parent().find("input.containerPort").focus();
			flag = 1;
