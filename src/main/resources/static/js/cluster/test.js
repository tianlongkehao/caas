...
function deployOneNode(obj){
	var teststatus = $(obj).parent().attr("teststatus");
	if(teststatus == "true"){
		layer.msg("该节点已安装部署完成，若重新部署需先清理部署！",{icon: 0});
		return;
	}
	...
	var deployHtml = '<div class="progress-bar progress-bar-striped active progress-bar-success " role="progressbar"'
		 ...
		 +'<span >正在部署</span>'
		 +'</div>';
	var thisNodeProgress = $(obj).parents(".thisTr").find(".nodeProgress");
	var thisClusterTestOpr = $(obj).parents(".thisTr").find(".clusterTestOpr");
	thisNodeProgress.empty().append(deployHtml);
	thisClusterTestOpr.children("a").empty();
	$.ajax({
		...
		success : function(data) {
			...
			if (data.status == '200') {
				...
				thisNodeProgress.empty().append(deployHtmlSuccess);
				thisClusterTestOpr.children("a").empty();
				$(obj).parent().attr("deployStatus","true");
				$(obj).parent().attr("teststatus","true");
				...
				var deployHtmlFailure = '<div class="progress-bar progress-bar-danger" role="progressbar"'
					 ...
					 +'<span >部署失败</span>'
					 +'</div>';
				thisNodeProgress.empty().append(deployHtmlFailure);
				thisClusterTestOpr.children("a").empty();
...
//测试默认值TODO
			...
			layer.open({
				...
				yes : function(index, layero) {
					...
					var nodeTestOpr = $(obj).parents(".thisTr").find(".clusterTestOpr");
					var successInfoBtn = '<a id="'+selectednode+'"  nodename="'+selectednode+'" onclick="detail(this)" title="查看详细信息">'
									+'<font color="#33CC33" style="font-weight:bold">通过<i class="fa fa-question-circle"></i></font></a>';
					var failInfoBtn = '<a id="'+selectednode+'"  nodename="'+selectednode+'" onclick="detail(this)" title="查看详细信息">'
					...
					nodeTestOpr.empty();
					$.ajax({
						url:ctx + "/cluster/deleteTestInfo?nodename="+selectednode,
						type:"get",
						success:function(){
							//1
...
function testNodes(obj){
	...
	if(obj == undefined){
		...
		//单独一个  选中的节点
		selectednode = $(obj).parent().attr("nodename");
