...
public class ApplicationFilterConfiguration {
    ...
    public FilterRegistrationBean filterRegistration() {
        ...
        registration.setEnabled(configProps.getEnable());
        registration.setFilter(ssoFilter());
        registration.setName("SSO Filter");
        registration.addInitParameter("serverName", configProps.getClientHostUrl()); // CAS客户端地址
        registration.addInitParameter("casServerUrlPrefix", configProps.getServerUrlPrefix()); //CAS服务器地址全路径
        registration.addInitParameter("casServerLoginUrl", configProps.getServerLoginUrl()); //CAS服务器登录地址 全路径
